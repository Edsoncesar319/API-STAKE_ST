...
# Helper functions (mesmas de messages.py)
...
def vercel_handler(req, res):
    ...
    try:
        ...
        if path.startswith('/api/budgets'):
            path = path.replace('/api/budgets', '/budgets', 1)
        ...
        if method == 'POST':
            ...
            try:
                ...
                db.rollback()
                _send_json_response(res, 500, {"error": f"Erro ao criar orçamento: {str(e)}"})
                return
            finally:
                db.close()
        ...
        # GET - Listar ou obter orçamento específico
        if method == 'GET':
            init_db()
        ...
        if method == 'PUT':
            ...
            try:
                ...
                db.rollback()
                _send_json_response(res, 500, {"error": f"Erro ao atualizar orçamento: {str(e)}"})
                return
            finally:
                db.close()
        ...
        if method == 'DELETE':
            ...
            try:
                ...
                db.rollback()
                _send_json_response(res, 500, {"error": f"Erro ao excluir orçamento: {str(e)}"})
                return
            finally:
                db.close()
